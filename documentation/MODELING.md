...
> **CELL-CONTAMINATION SCOPE NOTE (2026-09-09, applies to every grounder metric in this section)**: grounder `hc` records the FIELDED location, which partially encodes the outcome (out rate 85.5% when picked up in infield zones vs 0.7% in outfield zones — DATA_KNOWLEDGE). Every descriptive P(out | recorded cell) comparison on grounders — the engine's coverage surface AND the zone baselines, in every paragraph below — is therefore partially tautological on its ground component. These numbers remain valid for the descriptive estimand "P(out | recorded cell)" but are NOT evidence of prescriptive ranking power in either direction; prescriptive evidence comes from the realized-lift harness (Part 1) and the angular model's confirmation. Air-ball metrics are largely unaffected.

**Angular ground model shipped (2026-09-10) — current served system (0.5.0+cal-v2+ang-v1).** The ground game moved to the angular-corridor model (bearing-only, league stations, `angular_ground_v1.json`, LOSO slope 0.933); the RH direction regression below is RESOLVED — through the served path the delta ranking is validated for **both hands: L +0.344 / R +0.225** (delta computed under the batter's own unshrunk density; levels keep league shrinkage — never shrink deltas, EXPERIMENTS.md 2026-09-10). Absolute-level bias +0.0013.

**Frame revalidation (2026-09-09) — superseded old-frame numbers; RH finding since resolved (above).** In the corrected coordinate frame: direction validation read **LH +0.35 / RH −0.00** (the RH collapse was diagnosed as an engine-geometry/frame interaction and fixed by the angular model). The rebuilt zone×trajectory baseline improved to 0.5252 log loss, edging the calibrated surface (0.5283) with the engine keeping a thin AUC margin (+0.007 vs zone×traj; +0.0375 CI-separated vs plain zone) — **read this comparison through the contamination note above: both sides are inflated on grounders and it carries no prescriptive verdict.** The paragraphs below record old-frame results; they remain valid for the pre-2026-09-04 system only. [VERIFIED — EXPERIMENTS.md 2026-09-09/10]
...
**(Refreshed 2026-09-10; the 2026-09-02 version of this note is in git history — all three of its bottlenecks were resolved: shift over-claiming fixed, shift-selection falsified and closed, range model re-derived as the angular ground model.)** Current state: serving `0.5.0+cal-v2+ang-v1` — angular-corridor ground game (both-hands direction-validated), calibrated air surface, empirical landing densities (shrunk levels / unshrunk deltas). The remaining modeling levers, in order: (1) **optimizer-objective mismatch** — `optimize_positions` still searches legacy coverage geometry while its outputs are scored by the shipped calibrated path; aligning the search objective with the angular ground model is the next coherence fix. (2) **Fine x,y placement** stays UNKNOWN pending fielder-coordinate data (Gap 7). (3) **Batter-level absolute P(out)** stays permanently capped; the identified path (EV/trajectory-composition calibration) is parked P3. Do not trust descriptive grounder margins as prescriptive evidence (contamination note above); prescriptive claims run through the realized-lift harness. See `EXPERIMENTS.md`, `TRUST_REPORT.md`.
