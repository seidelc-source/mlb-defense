...
- **Known quirks & defects**:
  ...
  - **Grounder cell ↔ outcome contamination (2026-09-09)**: because grounder `hc` records the FIELDED location, the recorded cell partially encodes the outcome — grounders picked up in infield zones are outs **85.5%**, in outfield zones **0.7%** (2016–2022 counts). Consequence: any descriptive P(out | recorded cell) on grounders — the engine surface AND zone lookups alike — is partially tautological, and fielded-location DEPTH must not be used as prescriptive/counterfactual evidence; only the bearing (spray angle) is alignment-causally clean. Air balls largely unaffected. [VERIFIED — EXPERIMENTS.md 2026-09-09 angular-model entry]
