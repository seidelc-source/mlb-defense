...
## 2026-09-10 assessment (angular ground model SHIPPED — two-handed direction claim restored)

**Update**: the shipping experiment completed with all pre-registered gates passing (EXPERIMENTS.md 2026-09-10): serving is `0.5.0+cal-v2+ang-v1` — ground game on the angular-corridor model (league-level stations, bearing-only), air unchanged. Through the SERVED path the direction/ranking claim holds for **both hands: L +0.344 / R +0.225**. One disclosed structural amendment was required: league shrinkage on the landing density (kept for absolute levels, where it belongs) was injecting sample-size variance into cross-batter deltas — the decision delta now uses the batter's own smoothed unshrunk density. **Trust posture**: `oaa_delta` direction/ranking TRUSTED for L and R (magnitude approximate; switch-hitters unvalidated in the aggregate harness); absolute probabilities population-level only (unchanged, level bias +0.0013); shift-selection CLOSED (shift broadly); fine x,y placement UNKNOWN (Gap 7). Known residuals: optimizer's internal objective still legacy geometry (outputs re-scored by the shipped path); transfer assumption at shifted stations carries over. [VERIFIED]
