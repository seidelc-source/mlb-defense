...
- **Current trust status**: CONDITIONALLY TRUSTED for the calibrated P(out | standard-alignment) surface and handedness-corrected positioning direction. Shift-*selection* is CLOSED (2026-09-04): four candidate signals falsified, decision point A fired — the policy is "shift broadly" via per-batter optimized placement; `suggest_shift_type` is a template label only. Fine x,y placement UNKNOWN (data gap). See `documentation/TRUST_REPORT.md`.
